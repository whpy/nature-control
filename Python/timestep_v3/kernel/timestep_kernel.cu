...
void Qsolver::init(param pParam, int pBSZ, torch::Tensor &w, torch::Tensor &r1, torch::Tensor &r2, torch::Tensor &ra){
    if (initflag){
        cout << "init already" << "\n";
        return;
    }
    ...
    wold = new Field(mesh,(Qreal*)w.data_ptr());  wcurr = new Field(mesh); wnew = new Field(mesh); wnonl = new Field(mesh);
    // wold = new Field(mesh,w);  wcurr = new Field(mesh); wnew = new Field(mesh); wnonl = new Field(mesh);
    phi = new Field(mesh);
    u = new Field(mesh);
    v = new Field(mesh);

    r1old = new Field(mesh,(Qreal*)r1.data_ptr());  r1curr = new Field(mesh);  r1new = new Field(mesh);  r1nonl = new Field(mesh);
    // r1old = new Field(mesh, r1); r1curr = new Field(mesh);  r1new = new Field(mesh);  r1nonl = new Field(mesh);
    r2old = new Field(mesh, (Qreal*)r2.data_ptr()); r2curr = new Field(mesh);  r2new = new Field(mesh);  r2nonl = new Field(mesh);
    ...
    Ra = new Field(mesh, (Qreal*)ra.data_ptr());
...
// void Qsolver::Ravisual(){
//     cudaMemcpy(mesh->phys_h, Ra->phys, mesh->physize, cudaMemcpyDeviceToHost);
//     field_visual(mesh->phys_h, "Ra.csv", Nx, Ny);
// }
