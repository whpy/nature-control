...
Field *p11, Field *p12, Field *p21, Qreal Re, Qreal Er, Qreal lambda, Field *aux, Field* aux1){
    ...
    // no advection u*\nabla w now (commented out)
    // FldSet<<<mesh->dimGridp,mesh->dimBlockp>>>(wnonl->phys, 0.0, mesh->Nx, mesh->Ny, mesh->BSZ);
    FwdTrans(wnonl->spec, wnonl->phys, mesh);
...
inline 
void Incwnonl_func(Field* wnonl, Field *wcurr, Field *u, Field *v, Qreal Re, Field *aux, Field* aux1){
    Mesh *mesh = wnonl->mesh;
...
inline
void Inccurr_func(Field *wcurr, Field *u, Field *v){
    Mesh *mesh = wcurr->mesh;
    BwdTrans(wcurr->phys, wcurr->spec,mesh);
    vel_func(wcurr->spec, u->spec, v->spec, mesh);
    BwdTrans(u->phys, u->spec, mesh);
    BwdTrans(v->phys, v->spec, mesh);
...
__global__
void dealias(Qcomp *ft, Qreal *cutoff, int Nxh, int Ny, int BSZ){
    int i = blockIdx.x * BSZ + threadIdx.x;
    int j = blockIdx.y * BSZ + threadIdx.y;
    if (i<Nxh && j<Ny){
        int index = i + j*Nxh;
        ft[index] = ft[index]*cutoff[index];
    }
}
#endif