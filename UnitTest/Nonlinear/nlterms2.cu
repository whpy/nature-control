...
void nl1exact(Qreal *nl1, int Nx, int Ny, Qreal dx, Qreal dy){
    for (int i=0; i<Nx; i++){
        for (int j=0; j<Ny; j++){
            ...
            Qreal y = j*dy;
            int index = i + j*Nx;
            nl1[index] =  - 4*cos(x)*cos(y) + 4*sin(x)*sin(y) 
            + cos(2*x+y)*sin(x+y) + 0.4*sin(2*x+y) + 5*sin(x+y)*sin(2*x+y);
        }
    }
}
...
void nl2exact(Qreal *nl2, int Nx, int Ny, Qreal dx, Qreal dy){
    for (int i=0; i<Nx; i++){
        for (int j=0; j<Ny; j++){
            ...
            Qreal y = j*dy;
            int index = i + j*Nx;
            nl2[index] = -4 * cos(y) * sin(x)
           - 4 * cos(x) * sin(y)
           - 0.3 * sin(2 * x + y)
           - cos(x + y) * (cos(2 * x + y) + 5 * sin(2 * x + y));
        }
    }
}
int main(){
