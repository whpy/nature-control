...
void nl0exact(Qreal *nl0, int Nx, int Ny, Qreal dx, Qreal dy){
    for (int i=0; i<Nx; i++){
        for (int j=0; j<Ny; j++){
            ...
            Qreal y = j*dy;
            int index = i + j*Nx;
            nl0[index] = 5600*cos(5*x+y) - 13440*sin(5*x+y);
        }
    }
}
...
void nl1exact(Qreal *nl1, int Nx, int Ny, Qreal dx, Qreal dy){
    for (int i=0; i<Nx; i++){
        for (int j=0; j<Ny; j++){
            ...
            Qreal y = j*dy;
            int index = i + j*Nx;
            nl1[index] = -4 * cos(5 * x + y)
           + 2 * cos(17 * x + 3 * y) * sin(5 * x + y)
           + 298 * (0.0342282 + sin(5 * x + y)) * sin(17 * x + 3 * y);
        }
    }
}
...
void nl2exact(Qreal *nl2, int Nx, int Ny, Qreal dx, Qreal dy){
    for (int i=0; i<Nx; i++){
        for (int j=0; j<Ny; j++){
            ...
            Qreal y = j*dy;
            int index = i + j*Nx;
            nl2[index] = -4 * sin(5 * x + y) - 28 * sin(17 * x + 3 * y)
            - 2 * cos(5 * x + y) * (cos(17 * x + 3 * y) 
            + 149 * sin(17 * x + 3 * y));
        }
    }
}
